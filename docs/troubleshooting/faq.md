...
### How do I update DClaw Secure?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp secure --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Secure without Kubernetes?

Yes, for local development:

```bash
cd dclaw-secure/frontend && npm run dev
cd dclaw-secure/backend && uvicorn dclaw_secure.main:app --reload
```
