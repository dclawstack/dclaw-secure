...
import { Bot, Send, User, AlertCircle, Sparkles, Plus, MessageSquare } from "lucide-react";
...
export default function AIPage() {
  ...
  function sessionTitle(s: ChatSession): string {
    ...
    return firstUser ? firstUser.content.slice(0, 60) : "New chat";
  }
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col" style={{ height: "calc(100vh - 8rem)" }}>
      <div className="mb-4 flex items-start justify-between gap-3">
        ...
        <Button
          data-testid="new-chat-button-mobile"
          aria-label="New chat"
          variant="outline"
          size="sm"
          onClick={newChat}
      ...
      <aside className="hidden w-64 shrink-0 flex-col rounded-lg border bg-background md:flex">
        ...
        <div data-testid="session-list" className="flex-1 overflow-y-auto p-2">
          {sessions.length === 0 ? (
            <p className="px-2 py-4 text-center text-xs text-muted-foreground">No previous chats.</p>
          ) : (
              ...
              {sessions.map(s => (
                <li key={s.id}>
                  ...
                  >
                    ...
                    <span className="line-clamp-2 flex-1">{sessionTitle(s)}</span>
                  </button>
                </li>
              ))}
            </ul>
